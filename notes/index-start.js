...
function displayData(){
    ...
    objectStore.openCursor().onsuccess = function(e){
        ...
        if (cursor){
            ...
            let today = cursor.value.date;
            dateItem.textContent = today.getDate() + '/' + (today.getMonth()+1) + '/' +today.getFullYear() + ' ' + today.toLocaleTimeString();
...
function displayDataByDate(date){
    ...
    objectStore.openCursor().onsuccess = function(e){
        let cursor = e.target.result;
        if (cursor){
            if (cursor.value.date.toDateString() == date.toDateString()){
                let listItem = document.createElement('li');
                let h3 = document.createElement('h3');
                let para = document.createElement('p');
                let dateItem = document.createElement('span');
...
datePicker.onchange = function(e){
    if (e.target.valueAsDate !== null){
        displayDataByDate(e.target.valueAsDate);
    } else {
        displayData();
    }
